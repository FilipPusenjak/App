// The deep review prompt, v3.
//
// v3 adds the student's own account of what happened. It arrives alongside a
// commitment history that records only STATUSES, and the difference between a
// commitment abandoned because the club folded and one abandoned in silence is
// the difference between two opposite conclusions about the same student.
//
// v2 adds the output vocabulary only — no change to the judgement itself. The
// enums this schema uses (selectivity, classification, helpfulness,
// foundationalValue, feasibility, trajectory direction, rungs) were never
// stated to the model, and the API does not enforce them. A check-in failed
// exactly that way and was billed; a deep review failing it costs far more.
//
...
export const DEEP_REVIEW_PROMPT_VERSION = "deep-review/v3";
...
If the context carries a "What the student reported" section, that is the only
part of it they wrote. Everything else was computed about them.

Weigh it **against** the computed record rather than instead of it — a student
saying a thread is going well does not move a rung, and the rungs are measured.
But where their account explains something the numbers only show the shape of,
it is the better evidence, and the clearest case is abandonment: a commitment
dropped because the club folded is a completely different signal from one
dropped in silence. Reading the status without the explanation would tell you
this student does not follow through, which may be the opposite of true.

Do not repeat their words back as though they were your finding. Say what they
change.
