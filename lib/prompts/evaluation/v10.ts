...
// themselves rather than a reluctance to use them.
//
// The bands describe ACCUMULATED EVIDENCE: "several years-long commitments",
// "two or three commitments genuinely sustained". A Grade 9 student three
// months into secondary school cannot satisfy those words no matter what they
// do, so the upper bands were unreachable for a reason that has nothing to do
// with the student — which is precisely the stage error this prompt has been
// fixing since v5, reappearing inside the calibration text where the earlier
// fixes did not reach.
//
// It is also self-consistent nonsense: the pool is students in the SAME YEAR,
// and every one of them has had the same number of months. A requirement no
// member of the comparison group can meet cannot discriminate between them.
//
// v9 makes the bands stage-relative. "Sustained" means sustained for as long
// as the student has had — carried in from before, or joined at the first
// opportunity and still genuinely active. The floor is reinforced at the same
// time, because this is the change most at risk of becoming flattery: joining
// things and doing nothing in them is not commitment at any stage, and a
// one-off experience is not a commitment however impressive it sounds.
//
...
**Read those bands at the student's stage.** They describe accumulated evidence, and evidence takes time — so applied literally to a student three months into secondary school, the upper bands become unreachable for a reason that has nothing to do with the student. That is the stage error again, wearing a different hat: their entire comparison group has had exactly the same number of months.

"Sustained" therefore means sustained FOR AS LONG AS THEY HAVE HAD. In the early years that is: things carried in from before secondary school, things joined at the first opportunity and still running, and evidence they are actually doing something in them rather than appearing on a membership list. A Grade 9 student who joined several things in September and is genuinely active in them HAS sustained commitment by the only standard available to their year, and belongs in the 70-85 band on the strength of that plus top grades. Read "years-long" in the 85-95 band the same way — for a final-year student it means years; for a Grade 9 student it means since before they arrived.

Do not read that as a floor. Top grades with nothing sustained alongside them is a 50s or 60s profile, and a student coasting in easy courses is lower still, whatever their average looks like. Joining things and doing nothing in them is not sustaining anything at any stage, and one-off experiences are not commitments however impressive they sound. 90+ needs something that distinguishes them even within the strong group. This is a real measurement, not encouragement, and the low bands exist to be used.
...
export function buildUserPrompt(
  ...
  const { stable, variable } = buildUserPromptParts(snapshot, diff, reuse);
  return `${stable}\n\n${variable}`;
}
