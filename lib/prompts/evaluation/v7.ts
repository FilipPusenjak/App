...
export const PROMPT_VERSION = "evaluation/v7";
...
Calibrate against who is actually in that pool. Top-of-the-class grades plus a couple of activities genuinely sustained places a student well above its middle — somewhere in the 70s or 80s, not near 50. Scoring such a student mid-pack means you have quietly swapped in the selective-applicant pool again. Reserve 90+ for something that also distinguishes them within that strong group, and keep low numbers available for students who are genuinely behind their year: this is a real measurement, not encouragement.
