...
| `RETENTION_INPUT_SNAPSHOT_DAYS` | No | Days before the profile snapshot taken at each evaluation is deleted. Default 60, capped by the write-up window so it never outlives the assessment made from it. `0` keeps it forever. |
| `RETENTION_RESULT_DAYS_FREE` | No | Days before the evaluation write-up is deleted on the free plan. Default 30. `0` keeps it forever. |
| `RETENTION_RESULT_DAYS_PAID` | No | Days before the evaluation write-up is deleted on a paid plan. Default 365. `0` keeps it forever. **Scores are never deleted on any plan** — the progress chart still covers every year. |
| `RETENTION_POLICY_START` | No | When the tier split took effect, default `2026-09-10`. Evaluations written before it keep their write-up indefinitely on any plan, because they were written under the old 365-day promise. Their profile snapshots still expire at 60 days. Push it forward if the deploy slips. |
| `CRON_SECRET` | For triage, retention, reminders and digests | Bearer token the scheduled jobs authenticate with. `vercel.json` schedules two entries — the daily job (counselor triage, then the check-in nudge and the counselor triage digest) and the weekly retention sweep — and Vercel sets this header for you once the variable exists. **Fails closed: unset means no request is ever treated as the scheduler**, so triage only runs when a signed-in counselor asks for their own caseload, the retention sweep never runs at all — nothing is deleted — and no reminder or digest is ever sent. |
...
| `EMAIL_FROM` | With `RESEND_API_KEY` | The From address, on the domain you verified at <https://resend.com/domains>. A display name is optional: `CourseChart <hello@coursechart.app>`. |
